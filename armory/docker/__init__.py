...
from armory.utils.singleton import Singleton
from armory.logs import log, is_progress



class ImageMapper(metaclass=Singleton):
    """ Maps docker image names to their respective repositories and tags.
    """

    def __init__(self, *args, **kwargs):
        self._rpc = docker.from_env(version="auto")
        self.images = { c.attrs['RepoTags'][i]: c for c in self._rpc.images.list() for i in range(len(c.attrs['RepoTags'])) }

    @classmethod
    def resolve(cls, image_name):
        """ Return the full image name for the given image_name
        """
        instance = getattr(cls, '__instance', cls())
    ...
    def pull_image(self, image):
      ...
      try:
        self._rpc.api.pull(f"{image}:{tag}", tag=tag, stream=False)
